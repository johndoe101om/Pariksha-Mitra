...
// Survey of India sovereign coordinates for the full northern crown of Ladakh (matching user reference image)
const sovereignNorthCrown = [
  ...
  [73.00, 36.20], // Ishkoman / Yasin (top left wing)
  ...
  [77.84, 35.50]  // Connects to Aksai Chin in ring[0]
];
...
const fullFeatureCollection = {
  ...
  features: [
    ...geojson.features.filter(f => f.properties?.NAME_1 !== 'Jammu and Kashmir'),
    { type: 'Feature', geometry: { type: 'Polygon', coordinates: [ladakhRing] }, properties: { NAME_1: 'Ladakh' } },
    { type: 'Feature', geometry: { type: 'Polygon', coordinates: [jkRing] }, properties: { NAME_1: 'Jammu & Kashmir' } }
  ]
};
...
const projection = d3Geo.geoMercator()
  .fitExtent([[20, 25], [width - 20, height - 25]], fullFeatureCollection);
...
console.log('Projected Ladakh Centroid:', ladakhCentroid);
console.log('Projected JK Centroid:', jkCentroid);
...
const finalStates = OFFICIAL_ENTITIES.map((ent, idx) => {
  ...
  if (ent.name === 'Ladakh') {
    path = ladakhPath;
    centroid = [275, 80]; // Center of Ladakh
  } else if (ent.name === 'Jammu & Kashmir') {
    path = jkPath;
    centroid = [178, 105]; // Center of J&K
...
console.log('Successfully regenerated full sovereign Ladakh & India map in src/data/indiaMapData.ts!');
